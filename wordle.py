...
from colorama import init, deinit, Fore, Style #This import helps with color output

init(autoreset=True) # Colorama requires a init call. This is to add color to our output. Autoreset will reset the color of our output after every print
# List of possible secret words
word_list = ["crown", "logic", "plane", "stool", "tools", "plank", "fuzzy", "pizza", "about", "fizzy", "enter", "house", "speaker", "rocket", "movie", "light", "glass", "plant", "stand", "kitten"]
...
while new_game:
    ...
    print(f"The secret word has {game_word_c} lettes. You have {game_word_c} guesses remaning")
    guess_count = 0
    while True: # This will loop the game until we break out of it. (Number of guesses are up, or the player guesses right) 
        ...
        for x in range(0,len(guess_l)): # Loop through all the letters in the guess. 
            ...
            elif guess_l[x] in game_word_l: output += Fore.YELLOW + guess_l[x] # If the character is in the secret word but wrong place, add it to our output in orange
        ...
        if guess_count == game_word_c: # Player has run out of guesses
            ...
            print(f"The secret word was {Fore.GREEN}{game_word}")
            break

        if (game_word_c - guess_count) == 1: output += f"{Style.RESET_ALL}\t last guess!" # Print a unique message for last guess
